package com.app.ev119.domain.dto;

import com.fasterxml.jackson.annotation.JsonProperty;
import lombok.Data;

@Data
public class CheckAvailabilityIllPatientsItem {
    @JsonProperty("dutyName")
    private String dutyName;

    @JsonProperty("hpid")
    private String hpid;

    @JsonProperty("MKioskTy1")
    private String mKioskTy1;

    @JsonProperty("MKioskTy2")
    private String mKioskTy2;

    @JsonProperty("MKioskTy3")
    private String mKioskTy3;

    @JsonProperty("MKioskTy4")
    private String mKioskTy4;

    @JsonProperty("MKioskTy5")
    private String mKioskTy5;

    @JsonProperty("MKioskTy6")
    private String mKioskTy6;

    @JsonProperty("MKioskTy7")
    private String mKioskTy7;

    @JsonProperty("MKioskTy8")
    private String mKioskTy8;

    @JsonProperty("MKioskTy9")
    private String mKioskTy9;

    @JsonProperty("MKioskTy10")
    private String mKioskTy10;

    @JsonProperty("MKioskTy10Msg")
    private String mKioskTy10Msg;

    @JsonProperty("MKioskTy11")
    private String mKioskTy11;

    @JsonProperty("MKioskTy12")
    private String mKioskTy12;

    @JsonProperty("MKioskTy12Msg")
    private String mKioskTy12Msg;

    @JsonProperty("MKioskTy13")
    private String mKioskTy13;

    @JsonProperty("MKioskTy14")
    private String mKioskTy14;

    @JsonProperty("MKioskTy14Msg")
    private String mKioskTy14Msg;

    @JsonProperty("MKioskTy15")
    private String mKioskTy15;

    @JsonProperty("MKioskTy15Msg")
    private String mKioskTy15Msg;

    @JsonProperty("MKioskTy16")
    private String mKioskTy16;

    @JsonProperty("MKioskTy17")
    private String mKioskTy17;

    @JsonProperty("MKioskTy18")
    private String mKioskTy18;

    @JsonProperty("MKioskTy19")
    private String mKioskTy19;

    @JsonProperty("MKioskTy20")
    private String mKioskTy20;

    @JsonProperty("MKioskTy21")
    private String mKioskTy21;

    @JsonProperty("MKioskTy22")
    private String mKioskTy22;

    @JsonProperty("MKioskTy23")
    private String mKioskTy23;

    @JsonProperty("MKioskTy24")
    private String mKioskTy24;

    @JsonProperty("MKioskTy25")
    private String mKioskTy25;

    @JsonProperty("MKioskTy26")
    private String mKioskTy26;

    @JsonProperty("MKioskTy27")
    private String mKioskTy27;

    @JsonProperty("MKioskTy27Msg")
    private String mKioskTy27Msg;

    @JsonProperty("MKioskTy28")
    private String mKioskTy28;
}
